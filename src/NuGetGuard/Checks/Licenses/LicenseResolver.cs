...
/// <summary>
/// Identifies the licences the NuGet API did not state, and builds the licence section of the report.
/// Everything offline is tried first, so the network is only used for what is left.
/// </summary>
public static class LicenseResolver
{
    /// <summary>
    /// Names a licence from the shape of its URL, without a request. Must not fall back to
    /// <see cref="LicenseCatalog"/>: it is keyed by package id, so it would override the licence
    /// file of the scanned version, and packages do change their terms between versions.
    /// </summary>
    public static void ResolveFromUrlPatterns(IEnumerable<PackageMetadata> metadata)
    {
        foreach (var meta in metadata.Where(m => m.License == "Unknown"))
        {
            if (LicenseUrlResolver.ResolveFromUrlPattern(meta.LicenseUrl) is { } fromUrl)
                meta.License = fromUrl;
        }
    }

    /// <summary>Returns how many of the still unknown licences were identified.</summary>
