...
| `--fail-on` | `None` | `None` / `Vulnerable` / `Deprecated` / `Outdated` / `StrongCopyleft` / `Unused` / `Any` — exit code 1 when found |
| `--skip-redundant` | `false` | Skip the redundant-packages analysis |
| `--skip-unused` | `false` | Skip the unused-packages analysis |
| `--no-open` | `false` | Don't open the HTML report in the browser |

`--fail-on Any` covers vulnerable, deprecated, outdated and strong-copyleft findings. Unused packages are excluded from it and must be requested explicitly with `--fail-on Unused`, because that check is heuristic.
...
Deliberately conservative, so that a package in use is never called unused: build-time and analyzer packages are skipped, so are packages shipping no assemblies, framework polyfills, and — for `packages.config` projects — everything pulled in as someone else's dependency. Findings are still *candidates*: a package wired up purely through configuration, dependency injection or reflection has no namespace in the source and will be listed. Review before removing, and run `--skip-unused` to turn the check off.
...
━━━ 🧹 POSSIBLY UNUSED PACKAGES ━━━
  📂 MyApp.Worker  (SDK)
     🧹 AutoMapper 13.0.1  no code references AutoMapper, AutoMapper.Configuration, +4 more

  ⚠️  Heuristic — packages used only via configuration, DI or reflection also land here. Review before removing.
